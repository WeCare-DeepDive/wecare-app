...
public class PartnerAccessHandler extends AccessHandler {
    ...
    boolean isResourceOwner(Long resourceId) {
        ...
        if (currentMember.getRole() == Role.GUARDIAN) {
            isConnected = connectionRepository.existsByGuardianIdAndDependentIdAndActiveTrue(currentMember.getId(), resourceId);
        } else {
            isConnected = connectionRepository.existsByGuardianIdAndDependentIdAndActiveTrue(resourceId, currentMember.getId());
        }
